...
local IPROTO_DESCRIPTION_KEY = 0x2c
local IPROTO_FIELD_NAME_KEY = 0x2d
...
local method_codec           = {
    ...
    sql_execute = internal.encode_sql_execute,
...
function remote_methods:sql_execute(query, parameters, sql_opts, netbox_opts)
    check_remote_arg(self, "sql_execute")
    if sql_opts ~= nil then
        box.error(box.error.UNSUPPORTED, "sql_execute", "options")
    end
    ...
    local err, res, description = self._transport.perform_request(timeout,
                                    buffer, 'sql_execute', self.schema_version,
