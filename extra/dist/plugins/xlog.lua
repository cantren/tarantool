...
local function cat(ctx)
    ...
    if not stat then
        if type(rv) == 'string' then
            ...
            log.error("Failed to execute cat command")
            local rv_debug = nil
            if rv:match(':%d+: ') then
                rv_debug, rv = rv:match('(.+:%d+): (.+)')
            end
            log.error("Error occured at %s", rv_debug)
            log.error(rv)
        end
        return false
    end
...
local function play(ctx)
    local function basic_play(ctx)
        ...
        if not remote:wait_connected() then
            error("Failed to connect to host '%s'", uri)
        end
        for id, file in ipairs(ctx.files) do
            log.info("Processing file '%s'", file)
            for lsn, record in xlog.pairs(file) do
    ...
    if not stat then
        if type(rv) == 'string' then
            ...
            log.error("Failed to execute play command")
            local rv_debug = nil
            if rv:match(':%d+: ') then
                rv_debug, rv = rv:match('(.+:%d+): (.+)')
            end
...
local function xlog_prepare_context(ctl, ctx)
    ...
    if ctx.command == 'play' then
        ...
        if host == nil then
            log.error('Empty URI is provided')
            return false
        end
    ...
    for _, name in ipairs(ctx.positional_arguments) do
        if fio.stat(name) == nil then
            log.error("File %s expected to be read, but can't be found", name)
    ...
    if #ctx.files == 0 then
        log.error("No xlog/snap files are provided")
        return false
    end
    return true
end
